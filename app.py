...
st.markdown("##### 국가 공식 의약품 DB 실시간 연동 분석 시스템")
...
if st.button("실시간 상호작용 종합 분석 시작"):
    if not drug1:
        ...
        # 약 파일 내부에서 공식 사이트를 싹 뒤져서 결과를 가져옴!
        with st.spinner("🌐 식약처 공인 의약품 데이터베이스 실시간 조회 중..."):
        ...
        if status == "DANGER":
            ...
            st.warning("⚠️ 최종 판정 등급: 정보를 찾을 수 없음 (NOT FOUND)")
            st.info(f"💡 {reason_text}")
        else:
